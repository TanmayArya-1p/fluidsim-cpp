...
template <size_t N, size_t M> inline void clamp_point(defs::Point &point) {
  point.x = std::clamp<float>(point.x, 0, N - 1);
  point.y = std::clamp<float>(point.y, 0, M - 1);
}

template <size_t N, size_t M> void gaussian_blur(defs::grid<N, M> &grid) {
  defs::grid<N, M> temp = grid;

  constexpr float kernel[3][3] = {{1 / 16.0f, 2 / 16.0f, 1 / 16.0f},
