...
template <size_t sizeX, size_t sizeY> class FluidGrid {
...
public:
  FluidGrid(float _cell_size, float _density = 10.0f, float _viscosity = 0.01f)
      : density(_density), cell_size(_cell_size), viscosity(_viscosity) {
    this->vel_x.fill({});
    this->vel_y.fill({});
    this->pressure.fill({});
  }
  ...
  float compute_cell_pressure(const defs::Point &point) {
    ...
    return (p_sum - this->density * this->cell_size * vel_out_flow / DELTA_T) /
           ct;
  }
...
#pragma omp parallel for collapse(2)
      for (size_t x = 0; x < sizeX; ++x) {
        for (size_t y = 0; y < sizeY; ++y) {
          ...
          this->pressure[x][y] +=
              GAUSS_SEIDEL_SOR * (new_p - this->pressure[x][y]);
        }
      }
    }
  }
  ...
  float _interpolate_velocity_x(const defs::Point &pt) const {
    ...
    return (1 - fx) * (1 - fy) * vbl + fx * (1 - fy) * vbr +
           (1 - fx) * fy * vtl + fx * fy * vtr;
  }
...
#pragma omp parallel for collapse(2)
    for (size_t x = 0; x < sizeX + 1; ++x) {
      for (size_t y = 0; y < sizeY; ++y) {
        ...
        defs::Point prev_pos = {curr_pos.x - vel.x * DELTA_T,
                                curr_pos.y - vel.y * DELTA_T};
...
#pragma omp parallel for collapse(2)
    for (size_t x = 1; x < sizeX; ++x) {
      for (size_t y = 0; y < sizeY; ++y) {
        ...
        this->vel_x[x][y] -=
            K * (this->get_pressure(pt_right) - this->get_pressure(pt_left));
      }
    }
...
#pragma omp parallel for collapse(2)
    for (size_t x = 0; x < sizeX; ++x) {
      for (size_t y = 1; y < sizeY; ++y) {
        ...
        this->vel_y[x][y] -=
            K * (this->get_pressure(pt_top) - this->get_pressure(pt_bottom));
      }
    }
  }
