...
template <size_t sizeX, size_t sizeY> class FluidDisplay {
public:
  FluidDisplay(std::shared_ptr<solver::FluidGrid<sizeX, sizeY>> grid,
               size_t window_width, size_t window_height)
      : grid(grid), window_width(window_width), window_height(window_height) {
    ...
    for (size_t x = 0; x < sizeX; ++x) {
      for (size_t y = 0; y < sizeY; ++y) {
        if (grid->is_solid_cell({x, y})) {
          ...
          SDL_SetRenderDrawColor(renderer, intensity, intensity, intensity,
                                 255);
        }

        SDL_Rect cell = {
            static_cast<int>(x * cell_w), static_cast<int>(y * cell_h),
            static_cast<int>(cell_w) + 1, static_cast<int>(cell_h) + 1};
